...
$stmt = $pdo->prepare("SELECT u.id, u.username, t.id as trainee_id, t.name as trainee_name
                       FROM users u
...
if ($chat_with) {
    ...
    $stmt->execute([$faculty_id, $chat_with, $chat_with, $faculty_id]);
                    ...
                    <select id="traineeSelect" class="trainee-select">
                        ...
                        <?php foreach ($trainees as $trainee): ?>
                            <option value="<?php echo $trainee['id']; ?>" <?php echo $chat_with == $trainee['id'] ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($trainee['trainee_name']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                    ...
                    <?php if ($chat_with): ?>
                        ...
                        <div class="messages" id="messageContainer">
                            <?php foreach ($messages as $message): ?>
                                <div class="message <?php echo $message['sender_id'] == $faculty_id ? 'sent' : 'received'; ?>">
                    ...
                    success: function(response) {
                        ...
                        if (data.success) {
                            data.messages.forEach(function(message) {
                                appendMessage(message.id, message.message, message.sender_id == <?php echo $faculty_id; ?> ? 'sent' : 'received', message.timestamp);
